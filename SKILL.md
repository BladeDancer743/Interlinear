...
几何注释长度由术语的"广度 vs 深度"决定：

| 参数 | 触发条件 | 注释长度 |
|:--|:--|:--|
| `brief` | 通识术语（qubit, gate）、用户已多次见到 | ≤20 字 |
| `normal` | 论文首次出现的中等难度术语 | 20-35 字（默认） |
| `deep` | 对理解论文核心论证必不可少的概念、用户主动追问的术语 | 35-50 字 |

**默认 normal**。可在 §0.1b 交互中由用户指定：`「几何直觉模式——需要深一点的解释」` → 全篇使用 `deep`。

#### 术语类型 → 图像源自动映射

几何模式下，根据术语的**语义类别**自动选择最贴切的图像源。如果自动映射结果不满意，降级到下一个来源。

| 术语类型 | 首选图像源 | 原因 |
|:--|:--|:--|
| 门操作、酉变换、电路 | 1️⃣ 几何直觉 (Bloch球) | 本质上是球面上的旋转，天然几何 |
| 纠错码、稳定子、保护 | 1️⃣ 几何直觉 (高维空间) | 空间包围、轴系偏离的意象 |
| 纠缠、不可分性 | 4️⃣ 网络/层级 | 无法分离的捆绑结构 |
| 噪声、退相干、耗散 | 5️⃣ 日常力学 | 摩擦力、衰减的本质 |
| 算法加速、复杂度 | 2️⃣ 信息流动 | 路径缩近、信号放大 |
| 退火、优化、基态搜索 | 3️⃣ 概率/能量景观 | 滚球下山、山谷鞍点 |
| 密码、安全性 | 2️⃣ 信息流动 (锁与钥匙) | 信息访问控制 |
| 门保真度、错误率 | 1️⃣ 几何直觉 (角度偏差) | 旋转精度的几何度量 |
| 哈密顿量、能谱 | 3️⃣ 概率/能量景观 | 能量曲面的地形 |
| 张量网络、MPS/PEPS | 4️⃣ 网络/层级 | 网络节点缩并 |
| 量子态、密度矩阵 | 1️⃣ 几何直觉 (球面/球内) | 球面上的点 vs 球内的点 |
...
- **再现代用**：当术语在前文已有完整几何注释、后文再次出现时，简注 `【⤴简短翻译】` 即可——不重写几何图像。但如果再现处出现在与首次完全不同的论文语境中（如图像从 Bloch 球语境跳到了多体物理语境），可以换一个新的几何图像
- **同类统一**：同一类型的术语使用统一的图像风格。如果一个 qubit 门用 Bloch 球旋转描述，所有后续的门操作也都用球面旋转描述——不要让读者在球面旋转、水管开关、拼图块之间来回跳跃
...
- [ ] 是否出现数学符号或公式？（几何模式严禁）
- [ ] 是否引用了其他技术领域的术语做类比？
- [ ] 注释长度是否符合 depth 参数？（brief ≤20 / normal 20-35 / deep 35-50）
- [ ] 同类型术语是否用了相同风格的图像源？（门的注释都是球面旋转，噪声的注释都是耗散意象）
- [ ] 同一术语的后续简注 `【⤴】` 是否保持了与原注释一致的几何图像？
- [ ] 是否有术语该用几何图像却降级成了定义式？标记 `【🖼️暂无】`
